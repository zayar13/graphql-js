...
function parseDefinition(lexer: Lexer<*>): DefinitionNode {
  ...
  // Note: the Type System IDL is an experimental non-spec addition.
  if (
    peek(lexer, TokenKind.STRING) ||
    peek(lexer, TokenKind.MULTI_LINE_STRING)
  ) {
...
function parseTypeSystemDefinition(lexer: Lexer<*>): TypeSystemDefinitionNode {
  // Many definitions begin with a docstring and require a lookahead.
  const keywordToken =
    peek(lexer, TokenKind.STRING) || peek(lexer, TokenKind.MULTI_LINE_STRING) ?
      lexer.lookahead() :
      lexer.token;
